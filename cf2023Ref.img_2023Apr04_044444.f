...
\ File MD5 = 649C8F306CE629E514E5A0812BF1C912  roomed-zebra        
...
: chm ( -- ) #0 mhz ! md5b x ! #0 y ! #64 lblk ! $00010000
...
\ : keypad ( displays keypad and stack )
...
( icon number ) #28 MagentaV ic ( cursor ) #351 MagentaV cu 
...
: ok show black screen relay traffic keypad ;
: k show black screen relay keypad ;
...
: ok show blue screen text regs keypad ;
: cl $20 * $4AAB + ;
: buffer $2000 block ;
...
: ok show black screen text green return r r. blue r. r. white r. r. green r. r. drop stack keypad ; 
...
: ok show red screen text regs keypad ; 
...
\ Block 292 
( App: Test block : ) empty #0 MagentaV strt #62976 MagentaV lstup
: sze #256 block ;
...
: ok c cls act #0 pos ! show ch act $00 #650 at $00202020 color #1024 #768 box keypad ; 
...
\ ( F3 ) toggle seeb display of blue words ( - ) 
...
#4621480 MagentaV from 

#10308116 MagentaV tobl 

#298 MagentaV num 

#10000 MagentaV blkn
: tost ( u-- ) tobl @ ! #4 tobl +! ;
: ww ( -- ) #0 num ! #10000 blkn ! blkn @ block tobl ! 
   ftha from ! 
   fthn @ for from @ @ #9 or tost $25C7C00E tost 
   #4 from +! #1 num +! 
   num @ $06 mod #0 + 0if $9080000E tost then drop 
   num @ $06 #20 * mod #0 + mod 0if blkn @ #1 + blkn ! blkn @ block tobl ! #0 num ! then drop 
   next 
   #10000 edit 
...
\ ( demonstrates how to run colorforth words in one of the two background tasks ) 

\
\ : ttsrv1 ( counts up var1 )
\ : ttsrv2 ( counts up var2 ) 

\
\ : ksrv1 ( kills the serv1 task by giving it a null ) 
\    ( action )
\ : ksrv2 ( the same for serv2 )
\ : ttclr ( clears both counter variables )
\ : ttstop ( stops both tasks from running ) 
