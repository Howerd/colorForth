...
; 2drop and 2dup bug fix from Marco Nicola
...
displayTheKeypad:   ; the Keypad is the mnemonic at the bottom right of the display, showing the actions of each of the 27 keys used
    call setupText_
...
qring:  ; ( a cursor -- a' )  edi  contains pointer to current address to display
    _DUP_
    inc dword [ esi ]
    cmp [ v_curs ], edi
...
showShannonFano:    ; ( token -- ) \ display the Shannon-Fano encoded token on TOS
    ...
    and _TOS_, byte -0x10   ; and _TOS_, 0xFFFFFFF0  ignore the token colour when displaying the letters
...
.forward:
    ...
    jnz .forward2
    EMIT_IMM(':')   ; emit a ':' if in colourblind mode
    call space_
.forward2:
    jmp showSF_EDI_
...
editorKeypad:       ; the main editor keyboard icons
    db 'yrg*' ;               
    db 0x10, 0x11, 0x12, 0x13   ; 'ludr' arrow glyphs
    db '-mc+' ;                    
    db ' SCt' ;                    
    db 0x14,'f', 0x17, 'j'      ; "find" arrow glyphs + j
    db 'ab  ' ;                    
...
edit_:   ; ( n -- )   \ edit block n
    ...
    _DROP_                          ; discard n, and drop through to "e_"
e_:
...
showWords_:     ; ( -- )   \ show all words in the Forth wordlist
    ...
    mov ecx, [ v_ForthWordCount ]   ; count of Forth wordlist words
.loop:
    call showSF_EDI_            ; show one Shannon-Fano encoded word
    call space_
    inc edi
    loop .loop
    pop edi
...
; Looking at blocks 506 and 507 showed that the bug occurred between the two copy f.unctions
;
;    ; OK at this point
;    mov _SCRATCH_, [ v_blk ]
...
two_store_:     ; ( x1 x2 a -- )    \ 2!
   mov _SCRATCH_, [ _TOS_ ]         ;   
   mov ecx, [ esi + 4 ]             ; x1 into ecx
   mov [ _TOS_ ], _SCRATCH_         ; write x2 to addr+0
   mov [ _TOS_ + 4 ], ecx           ; and x1 to addr+4
   mov _TOS_, [ esi + 8 ]           ; read new TOS
   add esi, 12                      ; drop the stack 
   ret

plus_store_:  ; ( n addr -- )       \ +!
   mov _SCRATCH_, [ esi ]           ; copy the value n into the scratch register  
   add _SCRATCH_, [ _TOS_ ]         ; add to value at addr
   mov _TOS_, [ esi + 4 ]           ; refresh tos
   add esi, 8                       ; drop the stack
   ret
...
v_otherBlock:   ; the previously edited block
    dd 510 ; START_BLOCK_NUMBER + 1 ; the default other block is the shadow of the default edited block
...
v_quitMode:       ; if non zero, the keypad is in Edit mode, else TIB mode
    db 0            ; 255 enable, 0 disable
...
;    dd 0x18248800   ; rsect
    ...
    dd 0xA8AE2C80   ; graph
    dd 0x24CA4000   ; text
    dd 0xE893C4A0   ; keypa(d)  displayTheKeypad  was 0xE893C660 keybo(ard)
...
;    dd 0xC2295800   ; vregs
    ...
    dd 0xF8000000   ; @         fetch_          was replaced by optimising verson in block 70
    dd 0xD5F00000   ; 2@        two_fetch_      was replaced by optimising verson in block 70

    dd 0x97A00000   ; c!        cStore_
    dd 0xBFA00000   ; w!        wStore_
    dd 0xF4000000   ; !         store_          was replaced by optimising verson in block 70
    dd 0xF7E80000   ; +!        plus_store_
    dd 0xD5E80000   ; 2!        two_store_      was replaced by optimising verson in block 70
...
;    dd vregs_       ; vregs
    ...
    dd cFetch_      ; c@
    dd wFetch_      ; w@
    dd fetch_       ; @     was replaced by optimising verson in block 70
    dd two_fetch_   ; 2@    was replaced by optimising verson in block 70
    dd cStore_      ; c!
    dd wStore_      ; w!
    dd store_       ; !     was replaced by optimising verson in block 70
    dd plus_store_  ; +!
    dd two_store_   ; 2!    was replaced by optimising verson in block 70
...
font16x24:
; incbin "cf2022_font.img"
; incbin "cf2022Ref.img",( OFFSET_OF_FONT +(SIZE_OF_FONT_IN_BLOCKS *1024) ), ( 512 * 1024 ) ; append the font and colorForth source blocks from the reference image, skip the kernel code
; colorForth:                         ; the colorForth source blocks
incbin "cf2022Ref.img", OFFSET_OF_FONT, ( ( 512 - START_BLOCK_NUMBER + SIZE_OF_FONT_IN_BLOCKS ) * 1024 ); append the font and colorForth source blocks from the reference image, skip the kernel code
times 32768 db 'U'
; incbin "cf2022Ref.img", OFFSET_OF_FONT - (( 64 - 38 ) * 1024 ), ( ( 512 - START_BLOCK_NUMBER + SIZE_OF_FONT_IN_BLOCKS ) * 1024 ); append the font and colorForth source blocks from the reference image, skip the kernel code
